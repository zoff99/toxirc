...
static bool command_channel(Tox *tox, IRC *irc, uint32_t group_num, char *UNUSED(arg)) {
    ...
    tox_group_send_message(tox, group_num, TOX_MESSAGE_TYPE_NORMAL, (const uint8_t *)buffer, length, NULL, NULL);
...
static bool command_help(Tox *tox, IRC *UNUSED(irc), uint32_t group_num, char *UNUSED(arg)) {
    ...
    for (int i = 0; group_commands[i].cmd; i++) {
        ...
        tox_group_send_message(tox, group_num, TOX_MESSAGE_TYPE_NORMAL, (const uint8_t *)message, length, NULL, NULL);
    }
    return true;
}
